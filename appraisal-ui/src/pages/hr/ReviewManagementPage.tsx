...
export const ReviewManagementPage = () => {
  ...
  const load = () =>
    Promise.all([getAllReviews(), getAllAppraisals()])
      .then(([r, a]) => { setReviews(r); setAppraisals(a); })
              ...
              {reviews.map(r => {
                ...
                return (
                  <tr key={r.id} className="hover:bg-[#F4F8FF] transition-colors">
                    ...
                    <td className="table-cell">
                      {r.status !== 'APPROVED' && (
                        <button onClick={() => approve(r)} disabled={updating === r.id}
                          className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-green-50 text-green-700 hover:bg-green-100 text-xs font-medium transition disabled:opacity-50">
                          <Check size={12}/>{updating === r.id ? '...' : 'Approve'}
                        </button>
                      )}
                    </td>
                  </tr>
